...
const jwtSchema = Joi.object({
  ...
  cookie_names: Joi.array().items(Joi.string().required()).optional(),
  header_names: Joi.array().items(Joi.string().required()).optional(),
  claims_to_verify: Joi.array().items(Joi.string().required()).optional(),
